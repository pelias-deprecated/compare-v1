...
function MapController($scope, $rootScope, leafletData) {
  ...
  $rootScope.$on('geojson', (ev, geojson) => {
  ...
  $scope.addBoundingBoxes = function (i, geojson) {
    ...
    leafletData.getMap(`map${i}`).then((map) => {
      const bboxLayer = L.geoJson();
      (geojson.data.features || []).forEach((feat) => {
        if (feat.hasOwnProperty('bbox')) {
          const bounds = [[feat.bbox[1], feat.bbox[0]], [feat.bbox[3], feat.bbox[2]]];
